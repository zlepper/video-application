...
export function animateFrom(originalTarget: HTMLElement, options?: AnimateFromOptions) {
		...
		return () => {
			...
			return {
				...
				css: (t, u) => {
					...
					transform: ${transform} translate(${translateX * u}px, ${
						translateY * u
					}px) scaleX(${newScaleX}) scaleY(${newScaleY});
